<?php
	...
	function love() {
		...
		if ($conf) {
			$not = mysqli_query("INSERT INTO notificacoes (`userde`,`userpara`,`tipo`,`post`,`data`) VALUES ('$login_cookie','$userinfo','1','$publicacaoid','$data')");
	...
	function unlove() {
		...
		$conf = mysqli_query($del) or die(mysqli_error());
		if ($conf) {
			header("Location: index.php#".$publicacaoid);
		}else{
			echo "<h3>Erro</h3> ".mysqli_error();
...
<body>
	<div id="publish">
		<form method="POST" enctype="multipart/form-data">
			<br />
			<textarea placeholder="Escreva uma publicacão nova" name="texto"></textarea>
	...
	<?php
		while ($pub=mysqli_fetch_assoc($pubs)) {
			$email = $pub['user'];
			$saberr = mysqli_query("SELECT * FROM users WHERE email='$email'");
			$saber = mysqli_fetch_assoc($saberr);
			$nome = $saber['nome']." ".$saber['apelido'];
			$id = $pub['id'];
			/*$saberloves = mysqli_query("SELECT * FROM loves WHERE pub='$id'");
			...
			if ($pub['imagem']=="") {
				...
				<div id="love">';
				$email_check = mysqli_query("SELECT user FROM loves WHERE pub='$id' AND user='$login_cookie'");
